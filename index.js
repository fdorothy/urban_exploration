...
// howler music management
Howler.volume(document.getElementById('volume-control').value / 100.0)
let music = {
  url: '',
  sound: null
}
...
let img = (name) => {
  if (name) {
    println(`<img src="${name}" class="img"></img>`)
  }
}
...
let enterRoom = (id) => {
  ...
  if (room.name) {
    println(`<u>${getName(room.name)}</u>`, 'room-name');
  }
